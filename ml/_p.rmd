...
load("ORIG_DF_NO_PDROP.RData")
head(df)
#df <- df %>% filter(pid != "92511e53")
...
# Apply normalization for each participant
df <- df %>%
  group_by(pid) %>%
  mutate(elo_norm = normalize(elo))


length(unique(df$unique_id))
#[1] 1282
#sqrt(length(unique(df$unique_id))) -> 

# Create bins for normalized Elo ratings
df$elo_norm_bin = cut(df$elo_norm, breaks = 2, labels = FALSE)


df <- df %>% mutate(elo_norm_bin = as.factor(elo_norm_bin), 
                    wavelength = as.factor(wavelength),
                    probe = as.factor(probe), 
                    pid = as.factor(pid))
load("value ~ wavelength * probe * elo_norm_bin + (1|pid)_NOAVG.RData")
#model <- lmer(value ~ wavelength * probe * elo_norm_bin + (1|pid), data = df)
#save(model, "value ~ wavelength * probe * elo_norm_bin + (1|pid)_NOAVG.RData")
anova_results <- ezANOVA( data=df, dv = value, wid = pid, within = .(wavelength, probe, elo_norm_bin), detailed=TRUE, type=3 )

```

# visualize player elo puzzles played
```{r}
# Calculate mean Elo per player
player_means <- df %>%
  group_by(pid) %>%
  summarise(mean_elo = mean(elo)) %>%
  arrange(-mean_elo)

# Order players by mean Elo
df$pid <- factor(df$pid, levels = player_means$pid)

# Create histogram data
hist_list <- df %>%
  split(.$pid) %>%
  map(~ data.frame(counts = hist(.$elo, plot = FALSE)$counts, 
                   breaks = hist(.$elo, plot = FALSE)$breaks[-1], 
                   pid = unique(.$pid)))

# Create summary statistics
summary_df <- df %>%
  group_by(pid) %>%
  summarise(mean_elo = mean(elo), median_elo = median(elo))

# Merge summary statistics with histogram data
hist_df <- left_join(hist_df, summary_df, by = "pid")

# Create the plot
ggplot(hist_df, aes(x = breaks, y = counts)) +
  geom_col() +
  geom_vline(aes(xintercept = mean_elo), color = "blue", linetype = "dashed") +
  geom_vline(aes(xintercept = median_elo), color = "red", linetype = "dashed") +
  facet_wrap(~ pid, scales = "free_y") +
  labs(x = "Elo", y = "Count", title = "Elo histogram per player") +
  theme_minimal()
```
...
df_summary <- df %>%
  group_by(pid) %>%
  summarise(
    min_elo_bin = min(elo_bin, na.rm = TRUE),
    max_elo_bin = max(elo_bin, na.rm = TRUE),
    mean_elo_bin = mean(elo_bin, na.rm = TRUE),
    median_elo_bin = median(elo_bin, na.rm = TRUE),
    sd_elo_bin = sd(elo_bin, na.rm = TRUE)
  )

df %>%
  group_by(pid) %>%
  summarise(shapiro_test = shapiro.test(hist(elo_bin)$counts)$p.value) %>%
  mutate(normal = ifelse(shapiro_test > 0.05, "Yes", "No"))

df_norm <- df %>%
  group_by(participant_id) %>%
  mutate(
    elo_bin_mean = mean(elo_bin, na.rm = TRUE),
    elo_bin_sd = sd(elo_bin, na.rm = TRUE),
    elo_bin_normalized = (elo_bin - elo_bin_mean) / elo_bin_sd
  ) %>%
  ungroup()
...
df <- df %>% filter(elo_bin <= 1200) #1600)

# ANOVA
anova_results <- ezANOVA( data=df, dv = value, wid = pid, within = .(wavelength, probe, elo_bin), detailed=TRUE, type=3)
...
save(df, file="ORIG_DF_NO_PDROP.RData")
